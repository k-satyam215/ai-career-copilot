...
def test_get_llm_raises_without_key():
    ...
    with patch.dict(os.environ, {}, clear=True):
        ...
        with patch.dict(os.environ, env, clear=True), pytest.raises(
            RuntimeError, match="GROQ_API_KEY"
        ):
            get_llm()
    reset_llm_cache()
...
def test_get_llm_returns_client_with_key():
    ...
    reset_llm_cache()
    mock_groq = MagicMock()
    with patch.dict(os.environ, {"GROQ_API_KEY": "test-key-123"}), patch(
        "llm_provider.ChatGroq", return_value=mock_groq
    ):
        result = get_llm()
...
def test_get_llm_uses_default_model():
    from llm_provider import DEFAULT_MODEL, get_llm, reset_llm_cache
    reset_llm_cache()
